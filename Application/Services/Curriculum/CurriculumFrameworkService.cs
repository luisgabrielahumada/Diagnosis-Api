...
using Application.Services.Curriculum.Validate;
...
namespace Application.Services.Curriculum
{
    public interface ICurriculumFrameworkService
    {
        ...
        Task<ServiceResponse<ImportSummary>> ImportAsync(Guid id, ImportMccRequest file);
    ...
    public class CurriculumFrameworkService : ICurriculumFrameworkService
    {
        ...
        public async Task<ServiceResponse<ImportSummary>> ImportAsync(Guid id, ImportMccRequest req)
        {
            var sr = new ServiceResponse<ImportSummary>();
            ...
            // Sanity check: incoming context must match MCC context
            ...
            // Parse sheets -> raw rows (attempted)
            var compRows = ParseBasicSheet(wb, "Competence");                        // List<BasicImportRow>
            var perfRows = ParseBasicSheet(wb, "AcademicPerformance");               // List<BasicImportRow>
            var unitRows = ParseBasicSheet(wb, "AcademicUnit");                      // List<BasicImportRow>
            var objRows = ParseObjectivesSheet(wb, "AcademicObjective");            // List<ObjectiveImportRow>
            var knowRows = ParseKnowledgesSheet(wb, "AcademicEssentialKnowledge");   // List<KnowledgeImportRow>

            // Prepare summary
            var summary = new ImportSummary
            {
                Competences = { Attempted = compRows.Count },
                Performances = { Attempted = perfRows.Count },
                Units = { Attempted = unitRows.Count },
                Objectives = { Attempted = objRows.Count },
                Knowledges = { Attempted = knowRows.Count }
            };

            // Track matched file and increment import counter
            var mccFile = resp
                .Data
                .MainCurriculumFrameworkFile?
                .FirstOrDefault(t => t.FileName.Equals(req.File.FileName, StringComparison.OrdinalIgnoreCase));

            if (mccFile is not MainCurriculumFrameworkFile)
            {
                sr.AddError("Error","El archivo cargado es invalido");
                return sr;
            }
            ...
            try
            {
                ...
                // COMPETENCES
                // ============================================================
                var competences = compRows.Normalize(req.AcademicPeriodId);
                summary.Competences.Processed = competences.Count;
                summary.Competences.FilteredOut = summary.Competences.Attempted - summary.Competences.Processed;
                summary.Competences.IntendedNew = competences.Count(x => !x.Id.HasValue || x.Id == Guid.Empty);
                summary.Competences.IntendedUpdate = competences.Count - summary.Competences.IntendedNew;

                foreach (var r in competences)
                {
                    var isNew = !r.Id.HasValue || r.Id == Guid.Empty;
                    try
                    {
                        var entity = new CompetenceDto
                        {
                            ...
                            AcademicPeriodId = r.PeriodId
                        };

                        var result = await _competence.SaveAsync(r.Id, entity);
                        if (result.Status)
                        {
                            if (isNew) summary.Competences.NewOk++; else summary.Competences.UpdateOk++;
                        }
                        else
                        {
                            if (isNew) summary.Competences.NewFailed++; else summary.Competences.UpdateFailed++;
                ...
                // PERFORMANCES
                // ============================================================
                var performances = perfRows.Normalize(req.AcademicPeriodId);
                summary.Performances.Processed = performances.Count;
                summary.Performances.FilteredOut = summary.Performances.Attempted - summary.Performances.Processed;
                summary.Performances.IntendedNew = performances.Count(x => !x.Id.HasValue || x.Id == Guid.Empty);
                summary.Performances.IntendedUpdate = performances.Count - summary.Performances.IntendedNew;

                foreach (var r in performances)
                {
                    var isNew = !r.Id.HasValue || r.Id == Guid.Empty;
                    try
                    {
                        ...
                        if (result.Status)
                        {
                            if (isNew) summary.Performances.NewOk++; else summary.Performances.UpdateOk++;
                        }
                        else
                        {
                            if (isNew) summary.Performances.NewFailed++; else summary.Performances.UpdateFailed++;
                            sr.AddError(result.Errors.FirstOrDefault()?.ErrorMessage);
                        }
                    }
                    catch (Exception ex)
                ...
                // UNITS
                // ============================================================
                var units = unitRows.Normalize(req.AcademicPeriodId);
                summary.Units.Processed = units.Count;
                summary.Units.FilteredOut = summary.Units.Attempted - summary.Units.Processed;
                summary.Units.IntendedNew = units.Count(x => !x.Id.HasValue || x.Id == Guid.Empty);
                summary.Units.IntendedUpdate = units.Count - summary.Units.IntendedNew;
                ...
                foreach (var r in units)
                {
                    ...
                    try
                    {
                        ...
                        if (result.Status)
                        {
                            if (isNew) summary.Units.NewOk++; else summary.Units.UpdateOk++;

                            if (result.Data is Guid unitId)
                            {
                                // Bind objectives by Unit description -> set UnitId for matches
                                objRows = objRows.Select(o =>
                                {
                        ...
                        {
                            if (isNew) summary.Units.NewFailed++; else summary.Units.UpdateFailed++;
                            sr.AddError(result.Errors.FirstOrDefault()?.ErrorMessage);
                        }
                    }
                    catch (Exception ex)
                ...
                // ============================================================
                // OBJECTIVES (only those with UnitId set)
                // ============================================================
                var objCandidates = objRows.Where(t => t.UnitId is Guid).ToList();
                summary.Objectives.Processed = objCandidates.Count;
                summary.Objectives.FilteredOut = summary.Objectives.Attempted - summary.Objectives.Processed;
                summary.Objectives.IntendedNew = objCandidates.Count(x => !x.Id.HasValue || x.Id == Guid.Empty);
                summary.Objectives.IntendedUpdate = objCandidates.Count - summary.Objectives.IntendedNew;

                foreach (var r in objCandidates)
                {
                    var isNew = !r.Id.HasValue || r.Id == Guid.Empty;
                    try
                    {
                        var entity = new AcademicObjectiveDto
                        {
                            ...
                            IsActive = r.IsActive,
                            IsDeleted = r.IsDeleted,
                            AcademicUnitId = r.UnitId
                        };

                        var result = await _academicObjective.SaveAsync(r.Id, entity);
                        if (result.Status)
                        {
                            if (isNew) summary.Objectives.NewOk++; else summary.Objectives.UpdateOk++;

                            if (result.Data is Guid objectiveId)
                            {
                                // Bind knowledges by Objective description -> set ObjectiveId for matches
                                knowRows = knowRows.Select(o =>
                                {
                        ...
                        {
                            if (isNew) summary.Objectives.NewFailed++; else summary.Objectives.UpdateFailed++;
                            sr.AddError(result.Errors.FirstOrDefault()?.ErrorMessage);
                        }
                    }
                    catch (Exception ex)
                ...
                // ============================================================
                // KNOWLEDGES (only those with ObjectiveId set)
                // ============================================================
                var knowCandidates = knowRows.Where(t => t.ObjectiveId is Guid).ToList();
                summary.Knowledges.Processed = knowCandidates.Count;
                summary.Knowledges.FilteredOut = summary.Knowledges.Attempted - summary.Knowledges.Processed;
                summary.Knowledges.IntendedNew = knowCandidates.Count(x => !x.Id.HasValue || x.Id == Guid.Empty);
                summary.Knowledges.IntendedUpdate = knowCandidates.Count - summary.Knowledges.IntendedNew;

                foreach (var r in knowCandidates)
                {
                    var isNew = !r.Id.HasValue || r.Id == Guid.Empty;
                    try
                    {
                        var entity = new AcademicEssentialKnowledgeDto
                        {
                            ...
                            IsActive = r.IsActive,
                            IsDeleted = r.IsDeleted,
                            AcademicObjectiveId = r.ObjectiveId
                        };

                        var result = await _academicEssentialKnowledge.SaveAsync(r.Id, entity);
                        if (result.Status)
                        {
                            if (isNew) summary.Knowledges.NewOk++; else summary.Knowledges.UpdateOk++;
                        }
                        else
                        {
                            if (isNew) summary.Knowledges.NewFailed++; else summary.Knowledges.UpdateFailed++;
                            sr.AddError(result.Errors.FirstOrDefault()?.ErrorMessage);
                        }
                    }
                    catch (Exception ex)
            ...
            // Return the summary along with any collected errors
            sr.Data = summary;
            return sr;
        }
        ...
        public async Task<ServiceResponse<MainCurriculumFrameworkFileDto>> GenerateAsync(Guid id, MainCurriculumFrameworkInfoDto filter)
        {
            ...
            try
            {
                ...
                const int TEMPLATE_ROWS_DEFAULT = 5;

                void AppendTemplates<T>(ICollection<T> list, Func<T> factory, int rows = TEMPLATE_ROWS_DEFAULT)
                {
